...
.. admonition:: ``DSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   3. Compute the inverse of :math:`k`, using a random blinding value :math:`b_1` as:
      :math:`k^{-1} = (b_1 * k)^{-1} * b_1 \bmod q`
   4. Compute :math:`r=(g^k \bmod p) \bmod q`
   5. Compute :math:`s=k^{-1}*(x*r+m)\bmod q`. Computation of :math:`x*r+m` is blinded by computing it as
      :math:`(x*r*b_2+m*b_2)/b_2` with :math:`b_2 = {b_1}^2 \bmod q`
