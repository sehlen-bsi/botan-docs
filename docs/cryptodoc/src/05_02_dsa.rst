...
The DSA signature scheme is no longer approved by [FIPS-186-5]_.
Since Botan 3.13.0 the ``dsa`` module is marked as deprecated and is no
longer enabled by the BSI build policy.
The implementation generally follows [FIPS-186-4]_. It uses deterministic nonces according to  [RFC6979]_ if the
corresponding compilation module `rfc6979` in Botan is enabled.
...
the constructor ``DSA_PrivateKey(const DL_Group& group, const BigInt&
x)`` loads an existing private value. Since Botan 3.13.0 all DSA key
constructors, including the decoding constructors, reject a group
without subgroup order ``q`` or with a ``q`` shorter than 160 bits
(``check_dsa_group()``). The implementation operates as follows:
...
.. admonition:: ``DSA_PrivateKey()``
   ...
   **Input (summary of different overload of the constructor):**
   ...
   -  ``grp``: DL_Group (**Z**/*p*\ **Z**)\* : ``p``, generator ``g`` with order
      ``q``
   -  ``x``: private DSA parameter (optional)
   ...
   **Steps:**
   ...
   1. If the private value ``x`` was provided, use it. Otherwise,
      sample ``x`` as a random number :math:`2 \leq x<q` from ``rng`` using the algorithm
      described in Section :ref:`pubkey_param/rng` (see the
      :ref:`DH key generation section <pubkey/dh>` for the case of a ``q``
      outside the range of 160 to 384 bits).
   2. Compute public value ``y`` as :math:`g^x \bmod{p}`

      Optional verification of the generated key with
      ``check_key(RandomNumberGenerator& rng, bool strong)``.
      ``DSA_PrivateKey::check_key()`` performs the DL private key check
      described in the :ref:`DH key generation section <pubkey/dh>`
      (group verification and private element check), additionally
      requires :math:`x < q` and performs a sign/verify pairwise
      consistency check with SHA-256
      (``KeyPair::signature_consistency_check()``).
...
.. admonition:: ``DSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   3. Compute the inverse of :math:`k`, using a random blinding value :math:`b_1` with
      :math:`1<b_1<q` as:
      :math:`k^{-1} = (b_1 * k)^{-1} * b_1 \bmod q`
   4. Compute :math:`r=(g^k \bmod p) \bmod q`
   5. Compute :math:`s=k^{-1}*(x*r+m)\bmod q`. Computation of :math:`x*r+m` is blinded by
      computing it as :math:`(x*r*b_2+m*b_2)/b_2` with :math:`b_2 = {b_1}^2 \bmod q`
   6. If :math:`s=0 \lor r=0` applies, the algorithm terminates with an error.
...
conform. This cryptographic construct does not need a viable random number
generator during signature computation. Therefore, the blinding value
:math:`b_2` is always deterministically derived from :math:`b_1`. However,
the RFC6979 module is prohibited in the BSI module policy.
