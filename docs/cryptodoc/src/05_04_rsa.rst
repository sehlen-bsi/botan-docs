...
.. admonition:: ``RSA_PrivateKey()``
   ...
   **Steps:**
   ...
   1. The algorithm initially checks the passed key length: it must be
      at least 1024 bits, at most 16384 bits and a multiple of 8 bits
      (the upper bound and the multiple-of-8 requirement were added in
      Botan 3.12.0). If this is not the case, the function terminates
   ...
   7. Additional values needed for CRT-RSA are computed as follows.
      ...
      Note that since Botan 3.12.0 a subset of these conditions is
      already enforced whenever an RSA key is constructed or decoded: a
      public key is rejected with a ``Decoding_Error`` unless ``N`` is
      positive and odd with :math:`384 \leq len(N) \leq 16384` bits and
      ``e`` is odd with :math:`1 < e < N` and :math:`len(e) \leq 256`
      bits. A private key is additionally rejected unless
      :math:`d \geq 2`, :math:`p \geq 3`, :math:`q \geq 3`,
      :math:`p \neq q` and :math:`p \ast q = N`. In particular, RSA
      public keys shorter than 384 bits can no longer be loaded at all.
      Since Botan 3.13.0, decoding a key whose algorithm identifier is
      ``rsaEncryption`` additionally fails with a ``Decoding_Error`` if
      the parameters field is neither NULL nor absent.
...
.. admonition:: RSA decryption
   ...
   **Steps:**
   ...
   1. Verify that the ciphertext has exactly the byte length of the
      modulus ``N`` (required since Botan 3.13.0 in accordance with RFC
      8017, the successor of [RFC3447]_, Sections 7.1.2 and 7.2.2;
      previously only longer inputs were rejected) and that
      :math:`0 < ct < N`; otherwise the operation is rejected with a
      ``Decoding_Error`` (the range check was added in Botan 3.12.0).
   2. Blind ciphertext: :math:`ct' = ct*{m_e}^e \bmod N`
   3. Use Chinese Remainder Theorem (CRT) to decrypt ``ct'`` and retrieve ``m'``
   ...
   4. Unblind message ::math:`m = m' * m_d \bmod N`

**Remarks:**
 - After each blinding step, the blinding values change:
   :math:`m_e = m_e * m_e \bmod N`, :math:`m_d = m_d * m_d \bmod N`.
   This is performed at most 64 times (defined by ``Blinder::ReinitInterval``
   in :srcref:`[src/lib/pubkey/blinding]/blinding.h:37|ReinitInterval`).
   Afterwards, ``rng`` is used to generate new blinding values.
...
:srcref:`src/lib/pk_pad/enc_padding/eme_pkcs1/eme_pkcs.cpp`, in the methods ``pad`` and
``unpad``.
...
.. admonition:: ``EME_PKCS1v15::pad()``
   ...
   **Steps:**
   ...
   4. Fill the buffer with :math:`l - len(k) - 2` random non-zero bytes
...
.. admonition:: ``EME_PKCS1v15::unpad()``
   ...
   **Steps:**
   ...
   5. ``bad_input_m |= (m[1] != 0x02)``
   6. ``for (i = 2; i < |m|; i++)``

      -  ``delim_idx += (seen_zero_m == 0) & 1``
      -  ``seen_zero_m |= (m[i] == 0x00)``

   7. ``bad_input_m |= ~seen_zero_m``
...
:srcref:`src/lib/pk_pad/enc_padding/eme_oaep/oaep.cpp`, in the functions ``pad()`` and
``unpad()``.
...
:srcref:`src/lib/pk_pad/sig_padding/emsa_pssr/pssr.cpp` is recommended [TR-02102-1]_. The
...
:srcref:`src/lib/pk_pad/sig_padding/iso9796/iso9796.cpp`
Both implementations follow the specification [ISO-9796-2]_.
Since Botan 3.13.0 the ``iso9796`` module is marked as deprecated and is
no longer enabled by the BSI build policy. The same release fixed several
defects in the ISO 9796-2 verification routine: the trailer type (implicit
``0xBC`` or explicit hash identifier followed by ``0xCC``) must now match
the configured scheme instead of being inferred from the signature, a
message representative shorter than the modulus is padded with leading
zero bytes before parsing, the unused leading bits of the representative
are required to be zero, and the message capacity used for recovery is
computed consistently with the encoder.
...
To sign data, the function ``std::vector<uint8_t> sign(RandomNumberGenerator&
rng)`` is called after the message has been fed into the padding scheme via
``update()``. It applies the padding scheme to obtain the message
representative ``m`` and passes it to the internal function ``raw_op()``.
The RSA signature algorithm operates as follows:

.. admonition:: ``RSA_Signature_Operation::sign()``
   ...
   **Steps:**
   ...
   1. Verify that :math:`0 < m < N`.
      If the message to sign is zero or exceeds the modulus of the RSA
      private key, the algorithm terminates with a ``Decoding_Error``
      (the rejection of zero input was added in Botan 3.12.0).
...
``bool is_valid_signature(std::span<const uint8_t> sig)``, which recovers
the message representative via the internal function
``recover_message_repr()`` and hands it to the padding scheme for
verification. The recovery proceeds as follows:

.. admonition:: ``RSA_Verify_Operation::recover_message_repr()``
   ...
   **Steps:**
   ...
   1. Verify that the signature has exactly the byte length of the
      modulus ``N`` and that it is smaller than the modulus of the public
      key. I.e., if :math:`len(x) \neq len(N)` or :math:`x \geq N`, the
      algorithm throws a ``Decoding_Error``. The exact-length requirement
      stems from RFC 8017, Sections 8.1.2 and 8.2.2.
