...
parameters:
 ...
 - The base point :math:`G` must satisfy the curve equation
   :math:`G_y^2 = G_x^3 + a \cdot G_x + b \bmod p`.
...
:srcref:`[src/lib/pubkey/ec_group]/ec_group.cpp:564|EC_Group`). Previous
...
Since Botan 3.13.0, the check that the base point lies on the curve is
performed on every path that creates an ``EC_Group`` (i.e., also when decoding
explicit domain parameters from DER and in the deprecated constructor that
accepts a cofactor), independently of whether the "pcurves" or the legacy
implementation is used. The deprecated constructor additionally verifies that
:math:`p` and :math:`n` are (probable) primes, that the discriminant is
non-zero, and that :math:`a`, :math:`b`, :math:`G_x` and :math:`G_y` are
reduced modulo :math:`p`. Furthermore, ``EC_Group::verify_group()`` now checks
that :math:`G` lies on the curve and has order :math:`n` (by verifying
:math:`(n-1) \cdot G = -G`) without relying on the legacy implementation.
...
The following properties of these types were introduced in Botan 3.13.0:

 - ``EC_AffinePoint`` provides the dedicated deserialization functions
   ``deserialize_compressed()`` and ``deserialize_uncompressed()`` that accept
   exactly one SEC1 encoding (header byte ``0x02``/``0x03`` or ``0x04``,
   respectively) of the correct length. The generic ``deserialize()`` function
   additionally accepts, for backward compatibility, the deprecated hybrid
   encoding and the single zero byte encoding the point at infinity. Public
   points of EC keys decoded from an X.509 or PKCS #8 structure are accepted
   only in the compressed or uncompressed encoding, as required by RFC 5480.

 - Arithmetic operations, assignments and comparisons that involve
   ``EC_Scalar`` objects belonging to different groups are rejected with an
   error (or compare as unequal).

 - The curve-specific scalar objects of the "pcurves" implementation are
...
.. table:: Private Elliptic Curve Data Structures
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:909|AffineCurvePoint>`)          |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1000|ProjectiveCurvePoint>`)     |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:163|IntMod>`)                    |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1228|EllipticCurve>`)            |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1293|BlindedScalarBits>`)        |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1390|PrecomputedBaseMulTable>`)  |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1422|WindowedMulTable>`)         |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1538|WindowedMul2Table>`)        |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1091|add_mixed`) and
projective-projective additions (see
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1103|add`). Both
...
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1121|dbl`).
Additionally, iterated point doubling is provided (see
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1108|dbl_n`) with
...
bit length :math:`bitlength(n)/8` rounded up to the next word length of the target
...
Botan implements the hash-to-curve and hash-to-scalar functions of RFC 9380
(``EC_AffinePoint::hash_to_curve_ro()``, ``EC_AffinePoint::hash_to_curve_nu()``
and ``EC_Scalar::hash()``) using the ``expand_message_xmd`` message expansion
and the simplified SWU mapping. These functions are not used by any of the
algorithms described in this chapter. Hash to curve is supported for the
"pcurves" implementations of secp256r1, secp384r1 and secp521r1 and, since
Botan 3.13.0, for brainpool256r1, brainpool384r1, brainpool512r1 and
numsp512d1. Since Botan 3.13.0, the rules of RFC 9380 on the hash function
are enforced: the collision resistance of the hash function must be at least
:math:`\min(\lceil bitlength(n)/2 \rceil, 256)` bits (e.g., SHA-256 is
rejected for secp384r1) and the hash output length must not exceed its block
size. XOF-based message expansion is not implemented. Applications can query
``EC_Group::hash_to_curve_supported()`` to determine whether a given
combination of group and hash function is supported.
...
in :srcref:`src/lib/pubkey/ecc_key/ecc_key.cpp:154|EC_PrivateKey`. To generate a
...
.. admonition:: ``EC_PrivateKey()``
   ...
   **Steps:**
   ...
   2. Compute public point :math:`Q` as :ref:`point multiplication
      ...
      instead computed as :math:`d^{-1} \cdot G`. This is required for ECKCDSA
...
Since Botan 3.12.0, ``EC_PrivateKey::check_key()`` additionally performs
a pairwise consistency check: it recomputes the public point from the
private scalar (as :math:`d \cdot G`, or :math:`d^{-1} \cdot G` for
ECKCDSA/ECGDSA) and returns false if the result does not equal the
stored public point.

Since Botan 3.13.0, when decoding a private key from its PKCS #8 encoding, the
domain parameters may be taken from the optional ``parameters`` field of the
``ECPrivateKey`` structure (RFC 5915) if the ``AlgorithmIdentifier`` does not
carry them. If both are present, they must denote the same group; otherwise
the decoding fails with an error.
...
:srcref:`[src/lib/pubkey/ecdh]/ecdh.cpp:37|raw_agree`. The algorithm receives
...
:srcref:`[src/lib/pubkey/ecdsa]/ecdsa.cpp:163|raw_sign`):
...
.. admonition:: ``ECDSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   1. During the initialization the signature operation, sample a random blinding scalar :math:`1 \leq b_i < n` from ``rng``

   2. :math:`e = H` and truncate :math:`e` to be at most bitlength(:math:`n`) bits long.

   3. Generate parameter :math:`k` as a random number :math:`0 < k < n` using
   ...
   4. :math:`r = R_x \bmod n` where :math:`R_x` is the x-coordinate of the affine point
      :math:`R_{x,y} = k \cdot G` (see :ref:`pubkey/ecc/scalar_mul`). The computation
      of :math:`k \cdot G` is done with the use of scalar blinding.


   5. Compute :math:`k'^{-1} = (b_i \cdot k)^{-1}` using the blinding
      value :math:`b_i`. The inversion either uses Fermat's little theorem (:math:`a^{-1} = a^{p-2} \bmod n`) or a
      curve-specific addition chain if available. In the former case, the computation is realized with a variable time scalar exponentiation.

   6. Compute :math:`u=(d \cdot b_i) \cdot r + e \cdot b_i`. The braces indicate the order of evaluation.

   7. Compute :math:`s = k'^{-1} \cdot u` which amounts to :math:`(b_i \cdot k )^{-1} \cdot (d \cdot r + e) \cdot b_i = k^{-1} \cdot (d \cdot r + e)`.

   8. Square the blinding value :math:`b_i` to
      obtain new a blinding value :math:`b_{i+1}` which replaces :math:`b_i` in
      the next signature generation with the same signature operation instance. This
      is done to avoid re-sampling/re-inverting the blinding value. This is useful when using
      [RFC6979]_ as then there might not be a seeded random number generator available during the signature creation.

   9. ensure that :math:`r \neq 0` and :math:`s \neq 0`. Otherwise throw an exception.

   10. Return :math:`(r, s)`

**Remark:** If Botan is built with the RFC6979 module, it implements
deterministic ECDSA signatures, which are not covered by [TR-03111]_.
Note that the RFC6979 module is prohibited in the BSI module policy.

**Remark:** The blinding countermeasure was reworked in Dec 2025 in the PR https://github.com/randombit/botan/pull/5173.

Signature Verification
^^^^^^^^^^^^^^^^^^^^^^

The signature verification algorithm works as follows (see
:srcref:`[src/lib/pubkey/ecdsa]/ecdsa.cpp:244|verify`):
...
The signature generation algorithm works as follows (see
:srcref:`[src/lib/pubkey/eckcdsa]/eckcdsa.cpp:166|raw_sign`):
...
The signature verification algorithm works as follows (see
:srcref:`[src/lib/pubkey/eckcdsa]/eckcdsa.cpp:238|verify`):
