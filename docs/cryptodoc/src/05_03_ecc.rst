...
refer to the optimized implementation. The legacy implementation that is used for
custom curve parameters was the basis for previous revisions of this document.
...
.. [#ecc_domain_parameters]
   ...
   - :math:`a, b \in \mathbb{F}_p`: curve coefficients in short Weierstrass form:
     :math:`E_{a,b}: y^2 = x^3 + a \cdot x + b`
...
.. admonition:: ``EC_PrivateKey()``
   ...
   **Steps:**
   ...
   2. Compute public point ``Q`` as point multiplication :math:`d \cdot G`, where ``G`` is the
      ...
      instead computed as :math:`d^{-1} \cdot G`. This is required for ECKDSA and ECGDSA key
...
.. admonition:: ``ECDH_KA_Operation::raw_agree()``
   ...
   **Steps:**
      ...
      2. Calculate the shared secret point :math:`S = d \cdot Q_b` using a constant-time
...
.. admonition:: ``ECDSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   4. Sample a :math:`\lceil \frac{length(n)}{4} \rceil` bit long random blinding
...
.. admonition:: ``ECDSA_Verification_Operation::verify()``
   ...
   **Input:**
   ...
   -  EC_Publickey: ``Q``, curve group parameters (``a``, ``b``, ``p``, ``G``, ``n``, ``h = 1``)
...
The signature generation algorithm works as follows (see
:srcref:`[src/lib/pubkey]/eckcdsa.cpp:163|raw_sign`):
...
.. admonition:: ``ECKCDSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   2. Sample a :math:`\lceil \frac{length(n)}{4} \rceil` bit long random blinding
      ``mask`` from ``rng`` and compute :math:`k'=k+n \cdot mask`.
   3. Compute point :math:`W=(x_1,y_1)=k' \cdot G`.
...
The signature verification algorithm works as follows (see
:srcref:`[src/lib/pubkey]/eckcdsa.cpp:235|verify`):
...
.. admonition:: ``ECKCDSA_Verification_Operation::verify()``
   ...
   **Steps:**
   ...
   3. Compute point :math:`W=s \cdot Q+e \cdot G`
...
.. admonition:: ``ECGDSA_Signature_Operation::raw_sign()``
   ...
   **Input:**
   ...
   -  ``m``: raw bytes to sign (digest of the data hashed with a user-defined
      hash function)
   -  EC_Privatekey: ``d``, ``Q``, curve group parameters (``a``, ``b``, ``p``, ``G``, ``n``, ``h = 1``)
   -  ``rng``: random number generator
   ...
   **Steps:**
   ...
   1. Truncate ``m`` to be at most :math:`\lceil log_2(n) \rceil` bits long.
   2. Sample parameter ``k`` as a random number
      :math:`0 < k < n`
      from ``rng`` using the algorithm described in Section
      :ref:`pubkey_param/rng` .
   3. Sample a :math:`\lceil \frac{length(n)}{4} \rceil` bit long random blinding
      ``mask`` from ``rng`` and compute :math:`k'=k+n \cdot mask`.
   4. Compute point :math:`W=(x_1,y_1)=k' \cdot G`
   5. Set :math:`{r = x_{1}}\bmod n`
   6. Compute :math:`{s = {d \cdot {({{k \cdot r} - m})}}}\bmod n`.
   7. If :math:`s = {0 \vee r} = 0`
      applies, the algorithm terminates with an error.
   8. Return ECGDSA signature (r,s).
...
.. admonition:: ``ECGDSA_Verification_Operation::verify()``
   ...
   **Input:**
   ...
   -  ``m``: digest of message bytes (using a user-defined hash function)
   -  EC_Publickey: ``Q``, curve group parameters (``a``, ``b``, ``p``, ``G``, ``n``, ``h = 1``)
   -  (``r``, ``s``): ECGDSA signature
   ...
   **Steps:**
   ...
   3. Truncate ``m`` to be at most :math:`\lceil log_2(n) \rceil` bits long.
   4. Compute :math:`{v_{1} = {r^{- 1} \cdot m}}\bmod n`
      and :math:`{v_{2} = {r^{- 1} \cdot s}}\bmod n`.
   5. Compute point
      :math:`W = (x_1, y_1) = {{v_{1} \cdot G} + {v_{2} \cdot Q}}`
   6. Return ``true`` if :math:`r \equiv x_1 \bmod q` applies. Otherwise it returns ``false``.
