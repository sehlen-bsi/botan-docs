...
its parameters. All curves recommended in [TR-02102-1]_ are included.
...
For most common named curves [#supported_curves]_, Botan provides optimized
...
are not accessible by an application using the library. For further detail, please
...
.. table:: Private Elliptic Curve Data Structures
   ...
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ``IntMod``                                                                                          | Curve specific representation of an integer modulo a prime (either :math:`p` for field elements, or :math:`n` for scalar arithmetics). Provides fondational modular arithmetics algorithms and a customization point for curve specific reductions.  |
   ...
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ``BlindedScalarBits``                                                                               | Allows randomizing the scalar representation of :math:`k` by adding the curve's prime order :math:`n` random :math:`m` times (:math:`k + n*m`).                                                             --                                       |
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1450|BlindedScalarBits>`)        |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ``PrecomputedBaseMulTable``                                                                         | Stores a precomputed multiplication table to perform online multiplication by :math:`G` using a sequence of point additions of successive powers of the base point in a comb-like technique                                                          |
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1605|PrecomputedBaseMulTable>`)  |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ``WindowedMulTable``                                                                                | Allows precomputing a multiplication table for generic point multiplication using a fixed window of some bit length :math:`W`. The online phase performs only double and add operations                                                              |
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h:1693|WindowedMulTable>`)         |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ``WindowedMul2Table``                                                                               | Allows precomputating a multiplication table for 2-ary point multiplication as a windowed variant of "Shamir's Trick" of some window bit length :math:`W`.                                                                                           |
...
silent*, as it is typically used for signature verification where no parameter
is secret. Though, an additional implementation applying the same counter
measures as the ordinary scalar multiplication is available. This may be useful
when implementing cryptographic schemes such as sPAKE, but it is currently not
used in the elliptic curve algorithms outlined below.
...
.. admonition:: ``EC_PrivateKey()``
   ...
   **Input:**
   ...
   - ``ec_group``: curve group parameters (``a``, ``b``, ``p``, ``G``, ``n``, ``h = 1``)
...
.. admonition:: ``ECDH_KA_Operation::raw_agree()``
   ...
   **Input:**
   ...
   -  EC_Privatekey: :math:`d`, :math:`Q`, curve group parameters (``a``, ``b``, ``p``, ``G``, ``n``, ``h = 1``)
   ...
   **Steps:**
      ...
      2. Calculate the shared secret point :math:`S = d \cdot Q_b` (see :ref:`pubkey/ecc/scalar_mul`)
...
.. admonition:: ``ECDSA_Verification_Operation::verify()``
   ...
   **Input:**
   ...
   -  EC_Publickey: :math:`Q`, curve group parameters (``a``, ``b``, ``p``, ``G``, ``n``, ``h = 1``)
...
.. admonition:: ``ECKCDSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   3. Compute the witness :math:`r = h(x)` , where :math:`h` is the hash
   ...
   5. Compute :math:`s = d \ast ((k - (r \oplus m)) \bmod n)`.
...
.. admonition:: ``ECGDSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   1. Truncate ``m`` to be at most :math:`length(n)` bits long.
   2. Sample parameter ``k`` as a random number :math:`0 < k < n` from ``rng``
...
.. admonition:: ``ECGDSA_Verification_Operation::verify()``
   ...
   **Steps:**
   ...
   2. Truncate ``m`` to be at most :math:`length(n)` bits long.
   ...
   6. Return ``true`` if :math:`r \equiv x \bmod n` applies, otherwise ``false``.
