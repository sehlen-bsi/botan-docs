...
Round 4 NIST submission [CMCE-R4]_ and the Classic McEliece ISO Draft [CMCE-ISO]_. It
...
with a traditional key agreement mechanism such as RSA or ECDH.
...
Since Botan 3.13.0, secret-dependent bits within the decoding subroutine and
the parity check matrix arithmetic (e.g., the entries of the reconstructed
error vector :math:`e` and the code word bits used in the syndrome
computation) are represented as ``CT::Choice`` values instead of ``bool``.
This avoids that the compiler converts operations on these values into
...
They are collected in a ``Classic_McEliece_Parameters`` object, which includes
...
Since Botan 3.13.0, the constructors of ``Classic_McEliece_PublicKey`` and
``Classic_McEliece_PrivateKey`` that take an ``AlgorithmIdentifier`` (i.e.,
decoding from X.509 ``SubjectPublicKeyInfo`` or PKCS#8 structures) reject with
a ``Decoding_Error`` any algorithm identifier whose parameters field is not
empty, as the parameter set is identified solely by the OID.
...
:srcref:`[src/lib/pubkey/classic_mceliece]/cmce_keys_internal.cpp:152|generate`).
It works as follows:
...
The Classic McEliece encapsulation procedure (:srcref:`[src/lib/pubkey/classic_mceliece]/cmce_encaps.cpp:87|raw_kem_encrypt`)
...
The Classic McEliece decapsulation procedure (:srcref:`[src/lib/pubkey/classic_mceliece]/cmce_decaps.cpp:128|raw_kem_decrypt`)
