...
.. admonition:: ``evaluate_ocsp_response()``
   ...
   **Steps:**
   ...
   3. Establish trust in ``signing_cert`` following Section 4.2.2.2 of
      ...
      c) It passes a full X.509 path validation, performed by calling
         ``x509_path_validate()`` with ``signing_cert`` as the end entity
         certificate, the certificates delivered with the OCSP response and
         the certificates in ``cert_path`` as additional untrusted
         certificates, the trusted ``certstores``, the usage type *OCSP
         Responder*, and ``ref_time``. The usage type check requires
         ``signing_cert`` to have the *digitalSignature* or *nonRepudiation*
         key usage as well as the *id-kp-OCSPSigning* extended key usage; a
         violation is reported as OCSP_RESPONSE_MISSING_KEYUSAGE

      If the path validation in criterion c) fails, Return a set containing
      two status codes: the most severe status code of that path validation
      (``Path_Validation_Result::result()``, e.g. CERT_HAS_EXPIRED or
      ...
      rejected, the latter indicates that the OCSP response was rejected for
      this reason.
