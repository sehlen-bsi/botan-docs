...
-  ``set_key_mask_mode(Key_Mode=0 / Mask_Mode=1, Mask_MSB_Mode=1,
   Mask_LSB_Mode=2)``
-  ``set_chain_address(i)``
-  ``set_hash_address(i)``
-  ``set_ots_address(i)``
...
.. admonition:: WOTS+ leaf node key generation
   ...
   **Notes:**
   ...
   - All referenced methods above are implemented in the constructors of
     ``XMSS_WOTS_PublicKey`` and ``XMSS_WOTS_PrivateKey`` in
     :srcref:`[src/lib/pubkey/xmss]/xmss_wots.cpp`.
...
.. admonition:: XMSS key generation
   ...
   **Notes:**
   ...
   - The generation procedure is implemented in :srcref:`[src/lib/pubkey/xmss]/xmss_privatekey.cpp`.
     Since Botan 3.13.0, the ``XMSS_PrivateKey()`` constructor delegates the
     generation of the seeds and of the root node to
     ``XMSS_PrivateKey::generate_keygen_material()``, so that the public key
     part is constructed immutably from the result. The tree hashing is
     performed by the internal helper class ``XMSS_Tree_Builder`` in its methods
     ``tree_hash()`` and ``tree_hash_subtree()``; ``XMSS_PrivateKey::tree_hash()``
     is a thin wrapper around this helper that is also used during signing.
...
arrays, wrapped in an ASN.1 OCTET STRING for the encoded key formats.
Since Botan 3.12.0, decoding of encoded keys enforces a strict DER
encoding of this OCTET STRING wrapper (``BER_Decoder::Limits::DER()``);
non-canonical BER encodings are rejected.

.. _pubkey_signature/xmss/stateful_key_index_registry:

Stateful_Key_Index_Registry
---------------------------

Handling the safe and persistent state update of XMSS private keys is crucial.
Botan manages the XMSS private key states in a ``Stateful_Key_Index_Registry``, a
thread-safe "Singleton" object. That way, XMSS private key states are kept in
a centrally managed location during application execution. The implementations is found in the files

- :srcref:`src/lib/pubkey/stateful_key_index/stateful_key_index_registry.h`
- :srcref:`src/lib/pubkey/stateful_key_index/stateful_key_index_registry.cpp`

The ``Stateful_Key_Index_Registry`` singleton provides the following methods::
...
Here the ``KeyId`` is defined as follows::
...
The identity ``m_val`` is the SHA-256 hash over the length-prefixed inputs
``algo_name``, ``algo_params``, ``key_material_1`` and ``key_material_2``.
For XMSS, the ``algo_name`` is ``"XMSS"``, the ``algo_params`` is the big-endian
encoding of the 32-bit OID value, ``max_operations`` is the total number of
signatures :math:`2^h` of the parameter set, ``key_material_1`` is the private
seed, and ``key_material_2`` is the PRF.
The parameters uniquely identify the managed XMSS private key and the ``Stateful_Key_Index_Registry::reserve_next_index()`` method
returns a safe-to-use leaf index and increments the state for the next call.
The signing algorithm manipulates the XMSS state as needed *before* actually
signing with the respective WOTS+ leaf.
Since all accesses to the registry are serialized by a mutex, it is safe
to use the same XMSS private key in multiple threads of the same process. A new
private key calling ``Stateful_Key_Index_Registry::reserve_next_index()`` for the first time lazily initializes the state
variable to "0".

Since Botan 3.13.0, the registry enforces the following additional rules:

- The counter never increments past ``max_operations``. Once the counter has
  reached this maximum, ``reserve_next_index()`` returns an empty optional and
  the XMSS signing operation throws an ``Invalid_State`` exception ("one time
  signatures exhausted"); an exhausted key remains exhausted. Accordingly,
  a serialized private key whose stored leaf index equals :math:`2^h` is
  accepted and denotes an exhausted key, whereas a larger index is rejected
  with a ``Decoding_Error``.
- If the same key identity is registered with a differing
  ``max_operations``, the registry throws an ``Internal_Error`` instead of
  sharing the counter.
- The registry detects a ``fork()`` of the process: both the registry and each
  ``KeyId`` record the process ID at their creation. If either does not match
  the current process ID during a registry access, the registry latches
  closed for the remainder of the process and every subsequent access
  (including read-only queries such as ``current_index()``) throws an
  ``Invalid_State`` exception. This prevents the reuse of a leaf index in
  the parent and the child process, which share copy-on-write duplicates of
  the counters.

When serializing private keys (using ``XMSS_PrivateKey::private_key_bits()``),
the next unused WOTS+ leaf index is persisted (using ``Stateful_Key_Index_Registry::current_index()``) along with the private key data.
