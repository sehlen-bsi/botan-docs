...
For XMSS, the ``algo_name`` is ``"XMSS"``, the ``algo_params`` is the OID value, and ``key_material_1`` is the private seed, and ``key_material_2`` is the PRF.
...
returns a safe-to-use leaf index and ensures atomic incrementation of the state for the next call.
