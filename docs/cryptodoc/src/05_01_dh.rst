...
The Diffie-Hellman (DH) key exchange enables two parties to agree on a
shared secret over a public channel. Its security relies on the
hardness of the discrete logarithm problem, more precisely of the
computational Diffie-Hellman problem, in the underlying group.

Formally, both parties agree on a discrete logarithm group: the finite
multiplicative cyclic group (**Z**/*p*\ **Z**)*, where ``p`` is a large
prime, together with a generator ``g`` of the group or of a subgroup of
prime order ``q``, i.e. :math:`g^{q} \bmod p = 1`. The key exchange
then proceeds as follows:

1. Each party samples a secret exponent ``x`` with :math:`1 < x < q`
   (respectively :math:`1 < x < p-1` if no subgroup order ``q`` is
   specified) and computes its public value :math:`y = g^{x} \bmod p`.
2. The parties exchange their public values. In the following, the
   public value received from the other party is denoted :math:`y_b`.
3. Each party computes the shared secret :math:`s = y_b^{x} \bmod p`.

Both parties obtain the same secret since
:math:`(g^{x_b})^{x_a} = (g^{x_a})^{x_b} = g^{x_a x_b} \bmod p`, where
:math:`x_a` and :math:`x_b` denote the secret exponents of the two
parties. The symbols ``p``, ``g``, ``q``, ``x``, ``y``, :math:`y_b` and
``s`` are used with this meaning throughout the remainder of this
chapter.
...
.. admonition:: DL_Group()
   ...
   **Steps:**
   ...
   4. Verify that the selected generator ``g`` is a quadratic residue modulo
      ...
      Otherwise, ``g`` is set to 4, which is always a quadratic residue
      modulo ``p`` (being the square of 2) and thus has order ``q``.
   ...
   **Steps:**
   ...
   the algorithm described in Section A.1.1.2 in [FIPS-186-4]_. 
   Note that [FIPS-186-4]_ is superseded by [FIPS-186-5]_ which
   does not contain the referenced paragraph about prime number generation any more. 
   The implemented algorithm operates as follows.
...
**Remark:** [FIPS-186-5]_ no
longer specifies DSA as a signature algorithm. Hence, the usage of
...
Alternatively, the constructor ``DH_PrivateKey(const DL_Group& group,
const BigInt& x)`` loads an existing secret value ``x``, which is
rejected unless it passes ``DL_Group::verify_private_element()`` (see
below). The key generation algorithm operates as follows:

.. admonition:: ``DH_PrivateKey()``

   **Input (summary of different overload of the constructor):**

   -  ``rng``: random number generator
   -  ``grp``: ``DL_Group`` (**Z**/*p*\ **Z**)* defined by ``p``, generator ``g`` with order ``q``
   -  ``x``: private DH parameter (optional)
   ...
   **Steps:**
   ...
   1. If the private value ``x`` was provided, use it. Otherwise
      (``generate_private_dl_key()`` in
      :srcref:`src/lib/pubkey/dl_algo/dl_scheme.cpp`):

      -  If the group has a subgroup order ``q`` whose bit length is
         between 160 and 384, sample ``x`` uniformly from the interval
         :math:`[2, q)` using ``BigInt::random_integer()``.
      -  Otherwise, determine the needed exponent length by calling
         ``grp.exponent_bits()`` and sample a random number of that bit
         length from the random number generator ``rng`` as secret DH
         value ``x``.
   ...
   2. Compute ``y`` as :math:`g^x \bmod p` with ``g`` and ``p`` taken from the input group ``grp``.
      ...
      bool strong)``. Since Botan 3.12.0, ``DH_PrivateKey::check_key()``
      verifies the group (see below) and the private value ``x``
      (``DL_Group::verify_private_element()``); it fails if one of the
      ...
      ``DH_PublicKey::check_key()`` verifies the group and the public
      value ``y`` (``DL_Group::verify_public_element()``); it fails if
      one of the following conditions is met:

      -  :math:`y \leq 1`
      -  :math:`y \geq p - 1` (since Botan 3.13.0; previously only
         :math:`y \geq p` was rejected)
      -  :math:`q \neq 0 \; \land \; {y^{q} \bmod p} \neq 1`

      Note that neither check verifies the consistency of the key pair,
      i.e. :math:`y = g^{x} \bmod p` is not checked.

      The group verification (``DL_Group::verify_group()``) is skipped
      for Botan's builtin named groups unless ``strong`` is set. It fails
      ...
      -  Miller-Rabin primality test of ``p`` or ``q`` (if ``q`` is set)
         fails. The test is performed with the function ``is_prime()``
         with an error probability of :math:`2^{-128}`, resulting in 65
         test iterations for groups obtained from an external source and
         a reduced, bit-size-dependent iteration count for randomly
         generated groups.
      -  Lucas primality test of ``p`` or ``q`` fails.
      -  :math:`q = 0` and the group stems from an external source and
         :math:`g^{i} \bmod p = 1` for some :math:`2 \leq i < 100`
         (:math:`2 \leq i < 1000` if ``strong`` is set), i.e. the group
         order is very small.
...
from the peer has been accepted. Note that since Botan 3.12.0 basic
validity checks are already enforced when a key or group is constructed
or decoded: every ``DL_Group`` construction rejects parameters unless
``p`` is positive and odd with :math:`3 \leq len(p) \leq 16384` bits,
:math:`2 \leq g < p`, and, if ``q`` is present, ``q`` is positive and
odd with :math:`len(q) < len(p)`. Public keys are rejected at
construction/decoding time unless :math:`1 < y < p`; only the subgroup
membership check :math:`y^{q} \bmod p = 1` is deferred to
``check_key()``.
...
.. admonition:: ``DH_KA_Operation::raw_agree()``
   ...
   **Steps:**
   ...
   2. Verify that :math:`y_b` is valid. That is the case if :math:`1<y_b<p-1` applies. The algorithm
      terminates with an exception, if :math:`y_b` is invalid. Note that the
      value :math:`p-1` is only rejected since Botan 3.13.0; it generates the
      subgroup of order 2, so that accepting it would confine the shared
      secret to :math:`\{1, p-1\}` and leak the parity of ``x``. The
      accepted range now matches the interval :math:`2 \leq y_b \leq p-2`
      prescribed by NIST SP 800-56A.
