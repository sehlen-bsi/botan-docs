...
def create_pull_request(repo, branch):
    ...
    description = '\n'.join([
        ...
        "",
        "Typically, you will need to pull this branch (`{branch}`) to categorize and audit the newly added patches. It is perfectly fine to push into this pull request and merge it after a code review.",
        "If not merged, this pull request will be updated with new patches continuously.",
    ])
...
def main():
    ...
    if pr:
        ...
        run_git(["checkout", "--quiet", auditinfo.auditdoc_autoupdate_branch()])
    else:
        logging.info(f"Did not find an open auto-update Pull Request, creating a branch...")
        run_git(["checkout", "--quiet", "-B", auditinfo.auditdoc_autoupdate_branch()])
    ...
    if update_uncategorized_patches(audit, repo, args.output_topic, args.output_topic_title):
        run_git(["add", audit.get_topic_file_path(args.output_topic)])

    run_git(["commit", "--quiet", "--author=Audit Update Bot <actions@github.com>", "--no-gpg-sign", "-m", f"Automatic update to match patches until {new_target[0:7]}"])
    run_git(["push", "--quiet", "--set-upstream", "origin", auditinfo.auditdoc_autoupdate_branch()])
